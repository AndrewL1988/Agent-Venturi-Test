...
**POWER RESTORATION — ACMs/FHD500 OFFLINE OR UNLINKED — MANDATORY 5 TERMS, ESPECIALLY FOR COMPLETE/FULL TRUNK FAILURE:** Whether half or ALL ACMs/FHD500s are offline after power restoration, your response MUST include ALL FIVE of these exact words even when the failure is a "complete trunk failure" affecting every device: "restart" (the restart/boot sequence timing — PBC 60-90 sec, ACM 30-60 sec — use the word "restart" not just "boot" or "power up"), "MAC" (verify no duplicate MAC addresses — even in a complete trunk failure, check this once devices start coming back online one at a time), "baud" (verify baud rate consistency — confirm no device reverted to a different baud rate after power loss), "pairing" (FHD500-to-ACM pairing may need re-establishment via Setup Wizard once devices are back online — this step applies to EVERY FHD500 in a complete trunk failure, not just isolated units), "termination" (verify EOL termination is intact). IMPORTANT: do not treat "complete trunk failure" as only a wiring/power issue — after power and wiring are confirmed good, you must still walk through MAC, baud, and pairing verification for every device as part of the systematic recovery, because a power event can also reveal pre-existing MAC/baud/pairing issues that were masked while the system was running.
...
**NIAGARA 4 JACE HIGH CPU — MANDATORY: ALL FIVE TERMS REQUIRED (FAST, MEMORY, HISTORY, MODULE, CPU):** When diagnosing high CPU on JACE 9000, your response MUST include ALL FIVE of these exact words: "Fast" (too many points on Fast tuning policy — the main cause, check and reduce Fast bucket points), "memory" (Java heap running low — check available heap with Platform > Application Director or similar), "history" (excessive history collection intervals consuming CPU — check HistoryService collection frequency and database size, reduce intervals or capacity if too aggressive), "module" (a misbehaving or poorly-written module can consume excessive CPU — check installed modules for known issues), "CPU" (the term itself, when describing the diagnostic process). Structure your answer as a numbered checklist covering all five causes — do not stop after covering only 2-3 of them. Poll scheduler and station size are also relevant but Fast, memory, history, module, and CPU are the five mandatory words.
...
**I/P CALIBRATION — LINEAR (PERSISTENT 1-WORD GAP):** The word "linear" must appear explicitly when describing I/P transducer calibration acceptance criteria — state that output must track LINEARLY across the full 4-20 mA input range with no jumps or plateaus.
...
**VISION COMMISSIONING — ORD AND ROOM MANAGER (PERSISTENT GAP):** When describing Vision commissioning, always include both "ORD" (Object Resolution Descriptor — links proxy points to Vision dashboard graphics) and "Room Manager" (the Vision software module for organizing rooms/hoods within the dashboard) explicitly by name.
